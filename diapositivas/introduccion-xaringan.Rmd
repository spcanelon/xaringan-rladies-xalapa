...
# ¿Cómo podemos crear presentaciones con Rla ayuda de xaringan?
...
.pull-right[
> .morado-claro[A simple, in browser, Markdown driven slideshow tool] .fade[targeted at people who know their way around HTML and CSS]
...
![:scale 95%](img/ej-sin-css.png)
]

.pull-right[
...into HTML (content) with style! `r emo::ji("sunglasses")`
![:scale 95%](img/ej-con-css.png)
]
...
.left[
+ `r icon::fa("file-code")` the new Rmd file (i.e. `my-slides.Rmd`)

+ `r icon::fa("folder-open")` the `img` folder containing logo files

+ `r icon::fa("folder-open")` the `css` folder containing the custom `nhsr` theme files<br>.small[and an HTML file in charge of [adding the NHS logo to each slide](https://www.garrickadenbuie.com/blog/xaringan-tip-logo-all-slides)]
]
...
`r icon::fa("star", color = "#e5bf00")` Slide content starts immediately after the YAML `---`

.left-column[
The .morado-claro[nhsrtheme] xaringan template comes pre-loaded with a few things
]
.right-column[
- .morado-claro[A `setup` chunk] with a few libraries and some chunk options set for the whole slide deck

- Some example slides separated by three dashes (`---`)
  1. .morado-claro[Title slide with the `title-slide` class.]<br>Content is aligned with the `left` and the `bottom` of the slide
  1. .morado-claro[Inverse slide with the `inverse` slide class.]<br>Content is aligned with `middle` and `center` of the slide
  1. .morado-claro[Incremental slides with no specified slide class (default).]<br>Incremental steps are made using two dashes (`--`), and bullet points made using markdown (`*`)
]
...
### `r icon::fa("th-large")`&nbsp;&nbsp;with placement

--

### `r icon::fa("image")`&nbsp;&nbsp;pretty pictures

--

### <i class="fas fa-star"></i>&nbsp;&nbsp;and icons
...
Use the [icon `r emo::ji("package")`](https://slides.mitchelloharawild.com/icon/): ` r icon::fa("brain")`,<br>or the [fontawesome `r emo::ji("package")`](https://github.com/rstudio/fontawesome#the-fontawesome-package): ` r fontawesome:fa("brain")`
]
...
.right-column[
The easiest way to access them is with the icon `r emo::ji("package")`:<br>` r icon::fa("orcid")`
]

--

.left-column[.center[
`r icon::ii("ios-flask-outline")`<br>
[Ionicons](https://ionicons.com/)
]]
.right-column[
I haven't had as much luck with these, but you can use the icon `r emo::ji("package")` here as well: ` r icon::ii("ios-flask-outline")`
]
